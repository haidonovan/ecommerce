    ...
    <div className="rounded-2xl border border-slate-200 bg-slate-50 p-3">
      <div className="flex items-start justify-between gap-3">
        <button type="button" onClick={() => setShowNote((value) => !value)} className="min-w-0 text-left">
          ...
          <p className="mt-1 text-xs font-bold text-slate-500">
            Unit: {formatMoney(displayCurrency === "USD" ? item.price / exchangeRate : item.price, displayCurrency, exchangeRate, false)}
          </p>
        </button>
      ...
      <div className="mt-3 flex items-center justify-between gap-3">
        <div className="flex items-center rounded-full bg-white p-1">
          <button type="button" onClick={() => onUpdateQty(Math.max(0, item.qty - 1))} className="size-8 rounded-full bg-slate-100 font-black">
            -
          </button>
          <input
            value={item.qty}
          ...
          />
          <button type="button" onClick={() => onUpdateQty(item.qty + 1)} className="size-8 rounded-full bg-slate-100 font-black">
            +
          </button>
        </div>
        <p className="text-sm font-black">
          {formatMoney(displayCurrency === "USD" ? lineTotal / exchangeRate : lineTotal, displayCurrency, exchangeRate, false)}
        </p>
      </div>
